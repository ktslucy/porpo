			...
			<table>
				<tr>
					...
					<td><input type="text" id="id" name="id"
						placeholder="아이디를 입력해주세요" value="${user.getId()}" readonly /> </td>
				</tr>
				<tr>
					<th class="required">비밀번호</th>
					<td><input type="password" id="passwd" name="passwd"
						placeholder="비밀번호를 입력해주세요" value="${user.getPw()}" required /></td>
				</tr>
				<tr>
					<th class="required">이름</th>
					<td><input type="text" id="name" name="name"
						placeholder="이름을 입력해주세요" value="${user.getUserName()}" required /></td>
				</tr>
				<tr>
					<th class="required">이메일</th>
					<td><input type="email" id="email" name="email"
						placeholder="예 : marketkurly@kurly.com" value="${user.getEmail()}" required /> <a href=""
						class="btn_type2">중복확인</a></td>
				</tr>
				<tr>
					<th class="required">휴대폰</th>
					<td><input type="tel" id="phone" name="phone"
						placeholder="예 : 010-1234-5678" value="${user.getPhone()}"
						pattern="[0-9]{3}-[0-9]{4}-[0-9]{4}" required /></td>
				</tr>
				<tr>
					<th class="required">주소</th>
					<td><input type="text" id="address" name="address" value="${user.getAddress()}"
						placeholder="주소를 입력해주세요" required /></td>
				</tr>
			</table>
			<div>
				<input type="submit" id="join" value="수정하기" class="btn_type1">
			</div>
		</form>
	</div>
</div>
